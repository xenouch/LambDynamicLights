...
public final class PrideFlagShapes {
	...
	static {
		...
		register(Identifier.of("pride", "horizontal_stripes"), horizStripes = (colors, matrices, x, y, w, h) -> {
			...
			BufferBuilder bb = t.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
			for (int i = 0; i < colors.size(); i++) {
				int color = colors.getInt(i);
				float r = ((color >> 16) & 0xFF) / 255f;
				float g = ((color >> 8) & 0xFF) / 255f;
				float b = ((color >> 0) & 0xFF) / 255f;
				bb.vertex(mat, x, y + sh, 0).color(r, g, b, 1);
				bb.vertex(mat, x + w, y + sh, 0).color(r, g, b, 1);
				bb.vertex(mat, x + w, y, 0).color(r, g, b, 1);
				bb.vertex(mat, x, y, 0).color(r, g, b, 1);
				y += sh;
			}
			drawAndClear(bb, t);
		});
		register(Identifier.of("pride", "vertical_stripes"), (colors, matrices, x, y, w, h) -> {
			...
			for (int i = 0; i < colors.size(); i++) {
				...
				float g = ((color >> 8) & 0xFF) / 255f;
				float b = ((color >> 0) & 0xFF) / 255f;
				bb.vertex(mat, x, y + h, 0).color(r, g, b, 1);
				bb.vertex(mat, x + sw, y + h, 0).color(r, g, b, 1);
				bb.vertex(mat, x + sw, y, 0).color(r, g, b, 1);
				bb.vertex(mat, x, y, 0).color(r, g, b, 1);
				x += sw;
			}
			drawAndClear(bb, t);
		...
		register(Identifier.of("pride", "circle"), (colors, matrices, x, y, w, h) -> {
			...
			{
				BufferBuilder bb = tess.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
				int color = colors.getInt(0);
				float r = ((color >> 16) & 0xFF) / 255f;
				float g = ((color >> 8) & 0xFF) / 255f;
				float b = ((color >> 0) & 0xFF) / 255f;
				bb.vertex(mat, x, y + h, 0).color(r, g, b, 1);
				bb.vertex(mat, x + w, y + h, 0).color(r, g, b, 1);
				bb.vertex(mat, x + w, y, 0).color(r, g, b, 1);
				bb.vertex(mat, x, y, 0).color(r, g, b, 1);
				drawAndClear(bb, tess);
			}
			BufferBuilder bb = tess.begin(VertexFormat.DrawMode.TRIANGLE_FAN, VertexFormats.POSITION_COLOR);
			...
			for (int p = 0; p < 2; p++) {
				...
				float g = ((color >> 8) & 0xFF) / 255f;
				float b = ((color >> 0) & 0xFF) / 255f;
				bb.vertex(mat, cx, cy, 0).color(r, g, b, 1);
				for (int i = 0; i < 65; i++) {
					float t = (i / 64f);
					final float TAU = (float) (Math.PI * 2);
					bb.vertex(mat, cx + (MathHelper.sin(t * TAU) * rd), cy + (MathHelper.cos(t * TAU) * rd), 0).color(r, g, b, 1);
				}
			}
			drawAndClear(bb, tess);
		});
		register(Identifier.of("pride", "arrow"), (colors, matrices, x, y, w, h) -> {
			...
			MatrixStack.Entry mat = matrices.peek();
			Tessellator t = Tessellator.getInstance();
			BufferBuilder bb = t.begin(VertexFormat.DrawMode.TRIANGLES, VertexFormats.POSITION_COLOR);
			int color = colors.getInt(0);
			float r = ((color >> 16) & 0xFF) / 255f;
			float g = ((color >> 8) & 0xFF) / 255f;
			float b = ((color >> 0) & 0xFF) / 255f;
			bb.vertex(mat, x, cy + s, 0).color(r, g, b, 1);
			// yes, 1.5. the demisexual flag triangle appears to not be equilateral?
			bb.vertex(mat, x + (s * 1.5f), cy, 0).color(r, g, b, 1);
			bb.vertex(mat, x, cy - s, 0).color(r, g, b, 1);
			drawAndClear(bb, t);
		});
		...
		register(Identifier.of("pride", "progress"), (colors, matrices, x, y, w, h) -> {
			...
			for (int color : triangleColors) {
				...
				float b = ((color >> 0) & 0xFF) / 255f;
				bb.vertex(mat, x, cy + s, 0).color(r, g, b, 1);
				bb.vertex(mat, x + (s * 1.1f), cy, 0).color(r, g, b, 1);
				bb.vertex(mat, x, cy - s, 0).color(r, g, b, 1);
				s -= hm / 6;
			}
			drawAndClear(bb, t);
